...
class ModelCatalogCms extends Model {
	...
	public function getMostViewed($data = array()) {
		...
		if ($cache) {
			$product_data = $this->cache->get('product.mostviewed.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id'). '.' . (int)$this->config->get('config_customer_group_id') . '.' . (int)$data['limit']);
		}
		...
		if (!$product_data) {
			...
			if ($cache) {
				$this->cache->set('product.mostviewed.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id'). '.' . (int)$this->config->get('config_customer_group_id') . '.' . (int)$data['limit'], $product_data);
			}
		}
